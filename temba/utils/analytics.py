...
def gauge(event, value=None):
    ...
    if _librato:
        _librato.gauge(event, value, settings.METRICS_HOSTNAME)  # pragma: needs cover
...
def track(user, event, properties=None, context=None):  # pragma: needs cover
    ...
    # set our source according to our hostname (name of the platform instance, and not unique host)
